...
#pred.linear <- predict(best.linear, newdata = final.test.bin)
confusionMatrix(data = best.linear$fitted,
                reference = final$status[rowTrain])
...
#test error rate
pred.radial <- predict(best.radial, newdata = df[-rowTrain,])
...
```{r}
set.seed(1)
lasso.fit <- train(x = final.train.x.con, y = final.train.y.con,
              method = "glmnet",
